...
### 1. Arbitrary folder opening

This is underrated.

No repo setup. No `mkdocs.yml`. No project structure required.

Just: **open folder → runs.**

A random folder of notes from 2019? Works.
Someone else's repo you just cloned? Works.
Your `~/Documents/`? Works.

### 2. AI-native context awareness

Docusaurus doesn't care about:

- tokens
- prompt budget
- AI context quality

human-context is built entirely around that. Every file shows token counts so you know exactly what you're feeding your AI — before you feed it.

### 3. Zero friction

Many devs won't install. Won't build. Won't set up.

But they will **double-click an HTML file.**

That's the only distribution that works without asking anything of the user.
...
Đây là workspace local-first để hiểu và làm việc với các markdown knowledge base lớn trong AI workflow.

Dùng ngay docs markdown hiện có của bạn. **Không build. Không cài. Không server.**

---

Docs generator hoạt động như thế này:

```
viết docs → build site → deploy → publish
```

human-context hoạt động như thế này:
...
Ba thứ tool này có mà Docusaurus/MkDocs không có:

**1. Mở folder tùy ý** — không cần repo setup, không cần config, không cần cấu trúc project. Chỉ cần: mở folder → chạy.

**2. AI-native context awareness** — Docusaurus không quan tâm token là gì. human-context build toàn bộ quanh prompt budget: mỗi file hiện số token để bạn biết đang feed AI bao nhiêu.

**3. Zero friction** — Nhiều dev sẽ không cài, không build, không setup. Nhưng họ sẽ double-click file HTML.
...
AI của bạn đọc file `.md` — `CLAUDE.md`, thư mục `memory/`, thư mục `docs/`. Nhưng đồng nghiệp, người mới vào nhóm, và chính bạn sau 3 tháng — cần đọc bằng mắt người thật, trong trình duyệt thật.
...
**1. 任意文件夹打开** — 不需要 repo 配置，不需要构建配置，不需要项目结构。只需：打开文件夹 → 运行。

**2. AI 原生上下文感知** — Docusaurus 不关心 token。human-context 围绕 prompt budget 构建：每个文件显示 token 数量，让你知道要喂给 AI 多少。

**3. 零摩擦** — 很多开发者不会安装，不会构建，不会配置。但他们会双击一个 HTML 文件。
...
**human-context** 就是缺失的那一半。一个单独的 `.html` 文件。打开浏览器，选择文件夹，立即获得完整的阅读体验。无需服务器，无需 `npm install`，无需构建步骤。
