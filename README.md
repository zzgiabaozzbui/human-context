...
Bạn đang làm context engineering. Có `CLAUDE.md`, thư mục `memory/`, thư mục `docs/` với 40 file ghi lại mọi quyết định bạn đã đưa ra.

AI của bạn đọc tất cả. Hoàn hảo.

Rồi một đồng nghiệp join. Họ mở folder trong VS Code. Markdown thô. Không có điều hướng. Không biết file nào liên quan đến file nào.

Hoặc chính bạn quay lại sau 2 tuần. Vẫn vậy — nhưng lần này là docs của chính mình.

Đó là khoảng trống cần lấp.
...
你正在做 context engineering。你有 `CLAUDE.md`、`memory/` 目录、还有一个 `docs/` 目录，里面有 40 个文件记录了你做过的每一个决定。

你的 AI 能完美地读取所有这些内容。

然后一个队友加入了。他们在 VS Code 里打开这个目录。原始 Markdown。没有导航。不知道哪个文件和哪个文件有关联。

或者你自己两周后回来。同样的问题——只不过这次是你自己的文档。
